...
class NewsAPIConnection(BaseConnection[requests.session]):
    """
    Handles a connection with the News API and retrieves news articles.

    """

    def _connect(self, **kwargs) -> requests.session:
        """
        Initializes the connection parameters and creates a persistent requests.Session for connecting with the News API.
    ...
    def top_headlines(self, ttl: int = 3600, **kwargs) -> Optional[Dict[str, Any]]:
        """
        Retrieves Top-Headlines Articles in a specific country ('US' by default) and category from the News API.
        The results are cached for a period specified by ttl.
    ...
    def _make_api_request(self, url: str, params: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print('ValueError : ', e)
            st.error(f'News API Server Error')
            return None